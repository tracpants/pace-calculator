...
function createPaceAdjusterAccordion() {
	if (!state.lastResult) return '';
	
	const { type, data } = state.lastResult;
	
	// Only show pace adjuster for pace and time calculations (not distance)
	if (type !== 'pace' && type !== 'time') return '';
	
	// Get the original distance and current pace information
	let originalDistance, originalDistanceUnit, currentPaceSeconds;
	
	if (type === 'pace') {
		const distInput = document.getElementById("pace-distance");
		originalDistance = parseFloat(distInput.value);
		originalDistanceUnit = state.distanceUnit;
		// Use the calculated pace from the result
		currentPaceSeconds = state.distanceUnit === "km" ? data.pacePerKm : data.pacePerMile;
	} else if (type === 'time') {
		const distInput = document.getElementById("time-distance");
		originalDistance = parseFloat(distInput.value);
		originalDistanceUnit = state.distanceUnit;
		// Get the input pace that was used for the calculation
		currentPaceSeconds = getSegmentedPaceValue('time-pace');
	}
	
	// Calculate current pace in minutes and seconds for display
	const currentPaceMinutes = Math.floor(currentPaceSeconds / 60);
	const currentPaceSecondsRemainder = Math.floor(currentPaceSeconds % 60);
	
	return `
		<div class="mt-3 pt-3 border-t" style="border-color: var(--color-border-subtle);">
			<button 
				id="pace-adjuster-toggle" 
				...
				aria-expanded="false"
				aria-controls="pace-adjuster-content"
			...
			<div 
				id="pace-adjuster-content" 
				class="hidden mt-2 px-3 py-3 rounded-lg" 
				...
				<div class="space-y-3">
					<div>
						<label class="block text-xs font-medium mb-2" style="color: var(--color-text-secondary);">
							Current pace: ${calc.formatTime(currentPaceSeconds)} /${state.distanceUnit}
						</label>
						<label class="block text-xs font-medium mb-2" style="color: var(--color-text-secondary);">
						...
						<div class="input-segmented-container">
							<div class="input-segmented-group">
								<input
									type="number"
									id="adjuster-pace-minutes"
									class="input-segmented text-sm"
									placeholder="${currentPaceMinutes.toString().padStart(2, '0')}"
									value="${currentPaceMinutes}"
									min="0"
									max="59"
									inputmode="numeric"
									autocomplete="off"
									aria-label="Minutes"
								/>
								<label class="input-segment-label text-xs">MM</label>
							</div>
							<span class="input-separator">:</span>
							<div class="input-segmented-group">
								<input
									type="number"
									id="adjuster-pace-seconds"
									class="input-segmented text-sm"
									placeholder="${currentPaceSecondsRemainder.toString().padStart(2, '0')}"
									value="${currentPaceSecondsRemainder}"
									min="0"
									max="59"
									inputmode="numeric"
									autocomplete="off"
									aria-label="Seconds"
								/>
								<label class="input-segment-label text-xs">SS</label>
							</div>
							<span class="ml-2 text-xs" style="color: var(--color-text-tertiary);">/${state.distanceUnit}</span>
						</div>
						<div id="adjuster-pace-error" class="input-error hidden text-xs mt-1" role="alert"></div>
					</div>
					
					<div id="pace-adjuster-result" class="pt-2 border-t" style="border-color: var(--color-border-subtle);">
						<div class="flex justify-between items-center">
							<span class="text-sm font-medium" style="color: var(--color-text-secondary);">New finish time:</span>
							<span id="adjusted-time-display" class="text-sm font-mono font-semibold" style="color: var(--color-interactive-primary);"></span>
						</div>
						<div id="time-difference-display" class="flex justify-between items-center mt-1">
							<span class="text-xs" style="color: var(--color-text-tertiary);">Time difference:</span>
							<span id="time-diff-value" class="text-xs font-mono"></span>
						</div>
					</div>
				</div>
			</div>
...
function setupPaceAdjusterAccordion() {
	const adjusterToggle = document.getElementById('pace-adjuster-toggle');
	const adjusterContent = document.getElementById('pace-adjuster-content');
	const adjusterChevron = document.getElementById('pace-adjuster-chevron');
	const minutesInput = document.getElementById('adjuster-pace-minutes');
	const secondsInput = document.getElementById('adjuster-pace-seconds');
	
	if (!adjusterToggle || !adjusterContent || !adjusterChevron) return;
	
	// Toggle accordion
	adjusterToggle.addEventListener('click', () => {
		const isExpanded = adjusterToggle.getAttribute('aria-expanded') === 'true';
		
		if (isExpanded) {
			// Collapse
			adjusterContent.classList.add('hidden');
			adjusterContent.setAttribute('aria-hidden', 'true');
			adjusterToggle.setAttribute('aria-expanded', 'false');
			adjusterChevron.style.transform = 'rotate(0deg)';
		} else {
			// Expand
			adjusterContent.classList.remove('hidden');
			...
			setTimeout(() => {
				scrollToExpandedContent(adjusterContent);
			}, 100);
		}
	});
	...
	[minutesInput, secondsInput].forEach(input => {
		if (input) {
			input.addEventListener('input', updatePaceAdjusterResult);
			input.addEventListener('blur', updatePaceAdjusterResult);
		}
	});
...
function updatePaceAdjusterResult() {
	const minutesInput = document.getElementById('adjuster-pace-minutes');
	const secondsInput = document.getElementById('adjuster-pace-seconds');
	const adjustedTimeDisplay = document.getElementById('adjusted-time-display');
	const timeDiffValue = document.getElementById('time-diff-value');
	const errorElement = document.getElementById('adjuster-pace-error');
	
	if (!minutesInput || !secondsInput || !adjustedTimeDisplay || !timeDiffValue) return;
	
	// Parse the adjusted pace
	const paceValidation = calc.parsePaceFromSegments(minutesInput.value, secondsInput.value);
	
	if (!paceValidation.valid) {
		// Show error
		if (errorElement) {
			errorElement.textContent = paceValidation.message;
			errorElement.classList.remove('hidden');
		}
		adjustedTimeDisplay.textContent = '--:--';
	...
	// Get original calculation data
	const { type, data } = state.lastResult;
	let originalDistance, originalDistanceUnit, originalTimeSeconds;
	
	if (type === 'pace') {
		const distInput = document.getElementById("pace-distance");
		originalDistance = parseFloat(distInput.value);
		originalDistanceUnit = state.distanceUnit;
		originalTimeSeconds = getSegmentedTimeValue('pace-time');
	} else if (type === 'time') {
		const distInput = document.getElementById("time-distance");
		originalDistance = parseFloat(distInput.value);
		originalDistanceUnit = state.distanceUnit;
		originalTimeSeconds = data.totalSeconds;
	}
	
	// Calculate new time with adjusted pace
	const adjustedTimeSeconds = calc.calculateAdjustedTime(
		originalDistance,
	...
	// Update display
	adjustedTimeDisplay.textContent = calc.formatTime(adjustedTimeSeconds, true);
	
	// Calculate and show time difference
	const timeDiff = calc.calculateTimeDifference(originalTimeSeconds, adjustedTimeSeconds);
	const diffClass = timeDiff.isFaster ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400';
	const diffSymbol = timeDiff.isFaster ? '-' : '+';
	
	timeDiffValue.innerHTML = `<span class="${diffClass}">${diffSymbol}${timeDiff.formattedDifference} (${timeDiff.isFaster ? 'faster' : 'slower'})</span>`;
}

function scrollToExpandedContent(element) {
	if (!element || element.classList.contains('hidden')) return;
	
	// Get the element position and dimensions
	const elementRect = element.getBoundingClientRect();
	const viewportHeight = window.innerHeight;
	
	// Check if the bottom of the expanded content is visible
	const isBottomVisible = elementRect.bottom <= viewportHeight;
	
	// Only scroll if the bottom is cut off
	if (!isBottomVisible) {
		// Calculate how much we need to scroll to show the bottom with some padding
		const scrollOffset = elementRect.bottom - viewportHeight + 20; // 20px padding
...
function showResult(label, value, type = 'success') {
	...
	setTimeout(() => {
		...
		// Setup pace adjuster accordion if present
		setupPaceAdjusterAccordion();
